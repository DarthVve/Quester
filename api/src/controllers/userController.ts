...
export async function dash(req: Request, res: Response, next: NextFunction) {
    try {
        const user = await UserInstance.findOne({ where: { id: req.cookies.id }, include: [{ model: JournalInstance, as: 'JOURNAL' }, { model: PostInstance, as: 'POSTS' }] }) as unknown as { [key: string]: string };
