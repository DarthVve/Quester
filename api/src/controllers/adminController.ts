    ...
    try {
        ...
        const response = await UserInstance.findAndCountAll({
            limit, offset, include: [
                { model: JournalInstance, as: 'JOURNAL' },
...
export async function getUser(req: Request | any, res: Response, next: NextFunction) {
    try {
        const { id } = req.params;
        const entry: any = await UserInstance.findOne({ where: { id }, include: [{ model: JournalInstance, as: 'JOURNAL' }, { model: PostInstance, as: 'POSTS' }] });
...
export async function delUser(req: Request, res: Response, net: NextFunction) {
    try {
        const { id } = req.params;
        const user = await UserInstance.findOne({ where: { id }, include: [{ model: JournalInstance, as: 'JOURNAL' }, { model: PostInstance, as: 'POSTS' }] });
        if (!user) {
